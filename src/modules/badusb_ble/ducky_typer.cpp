...
struct BadUsbDuckyCommand {
    const char *command;
    char key;
    DuckyCommandType type;
};

struct BadUsbDuckyCombination {
    const char *command;
    char key1;
    char key2;
    char key3;
};
const BadUsbDuckyCombination duckyComb[]{
...
const BadUsbDuckyCommand duckyCmds[]{
...
void key_input(FS fs, String bad_script, HIDInterface *_hid) {
    ...
    while (payloadFile.available()) {
        ...
        for (i = 0; i < repeatCount; i++) {
            BadUsbDuckyCommand *PriCmd = findDuckyCommand(Cmd);
            BadUsbDuckyCommand *ArgCmd = findDuckyCommand(Argument.c_str());
            ...
            if (PriCmd != nullptr) {
                ...
                // Combinations are processed here
                    ...
                    BadUsbDuckyCombination *comb = findDuckyCombination(Cmd);
                    if (comb != nullptr) {
...
BadUsbDuckyCommand *findDuckyCommand(const char *cmd) {
    for (auto &cmds : duckyCmds) {
        if (strcmp(cmd, cmds.command) == 0) { return const_cast<BadUsbDuckyCommand *>(&cmds); }
    }
    return nullptr;
}

BadUsbDuckyCombination *findDuckyCombination(const char *cmd) {
    for (auto &comb : duckyComb) {
        if (strcmp(cmd, comb.command) == 0) { return const_cast<BadUsbDuckyCombination *>(&comb); }
    }
    return nullptr;
}
