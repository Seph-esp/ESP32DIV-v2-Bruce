...
static void applyJamConfig(const NrfJamConfig &cfg, bool flooding) {
    ...
    auto configureRadio = [&](RF24 &radio) {
        radio.setPALevel(paLevels[cfg.paLevel & 3]);
        if (!radio.setDataRate(dataRates[cfg.dataRate <= 2 ? cfg.dataRate : 1])) {
    ...
    if (nrf1_online) configureRadio(NRFradio);
    if (nrf2_online) configureRadio(NRFradio2);
    if (nrf3_online) configureRadio(NRFradio3);
}
...
static void floodChannel(uint8_t ch, uint16_t dwellMs, uint8_t burstCount) {
    ...
    if (nrf2_online) {
        digitalWrite(47, LOW);
        NRFradio2.flush_tx();
        NRFradio2.setChannel(ch);
    }
    if (nrf3_online) {
        digitalWrite(14, LOW);
        NRFradio3.flush_tx();
        NRFradio3.setChannel(ch);
    }

    // 2. CE HIGH to start transmission
    if (nrf1_online) digitalWrite(bruceConfigPins.NRF24_bus.io0, HIGH);
    if (nrf2_online) digitalWrite(47, HIGH);
    if (nrf3_online) digitalWrite(14, HIGH);

    if (dwellMs == 0) {
        // Turbo: fill FIFO for maximum duty cycle
        for (int i = 0; i < burstCount; i++) {
            if (nrf1_online && !NRFradio.writeFast(JAM_FLOOD_DATA, 32, true)) {
                NRFradio.flush_tx();
            }
            if (nrf2_online && !NRFradio2.writeFast(JAM_FLOOD_DATA, 32, true)) {
                NRFradio2.flush_tx();
            }
            if (nrf3_online && !NRFradio3.writeFast(JAM_FLOOD_DATA, 32, true)) {
                NRFradio3.flush_tx();
            }
        }
        delayMicroseconds(200);
        if (nrf1_online) digitalWrite(bruceConfigPins.NRF24_bus.io0, LOW);
        if (nrf2_online) digitalWrite(47, LOW);
        if (nrf3_online) digitalWrite(14, LOW);
        return;
    }
...
static void turboFloodChannel(uint8_t ch, uint8_t burstCount) {
    ...
    for (int i = 0; i < burstCount; i++) {
        if (nrf1_online && !NRFradio.writeFast(data, 32, true)) NRFradio.flush_tx();
        if (nrf2_online && !NRFradio2.writeFast(data, 32, true)) NRFradio2.flush_tx();
        if (nrf3_online && !NRFradio3.writeFast(data, 32, true)) NRFradio3.flush_tx();
    }
...
static void initCW(int channel) {
    ...
    if (nrf1_online) setupCW(NRFradio);
    if (nrf2_online) setupCW(NRFradio2);
    if (nrf3_online) setupCW(NRFradio3);
}
...
static void cwChannel(uint8_t ch, uint16_t dwellMs) {
    if (nrf1_online) NRFradio.setChannel(ch);
    if (nrf2_online) NRFradio2.setChannel(ch);
    if (nrf3_online) NRFradio3.setChannel(ch);
...
static void runJammer(NRF24_MODE nrfMode, NrfJamMode jamMode) {
    ...
    while (running) {
        ...
        if (check(SelPress)) {
            if (CHECK_NRF_SPI(nrfMode)) {
                if (nrf1_online) NRFradio.stopConstCarrier();
                if (nrf2_online) NRFradio2.stopConstCarrier();
                if (nrf3_online) NRFradio3.stopConstCarrier();
            }
        ...
        // ── Periodic display update (every 200ms) ────────────
        {
            ...
            if (nowMs - lastDrawMs >= 200 || redraw) {
                // Sum online module counts
                int totalOnline = 0;
                if (nrf1_online) totalOnline++;
                if (nrf2_online) totalOnline++;
                if (nrf3_online) totalOnline++;
                drawJammerStatus(currentMode, channel, totalOnline, true, nowMs - jamStartMs, totalCh, sweepCount);
                lastDrawMs = nowMs;
                redraw = false;
            }
        }
